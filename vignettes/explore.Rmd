...
```{r fig.width=7, fig.height=5}
correctness %>%
  gather(algo, item, ccrank, crank, rank, rej, R) %>%
  ggplot(aes(x=factor(item),y=j,color=algo)) +
  geom_boxplot() +
  facet_wrap(~prob)
```
...
```{r}
correctness %>%
  gather(algo, item, ccrank, crank, rank, rej, R, R2) %>%
  group_by(prob, algo, r, n, j, item) %>%
  summarize(count = n()) %>%
  ungroup %>%
  spread(algo, count) %>%
  gather(algo, other_count, ccrank, crank, rank, rej) %>%
  mutate(rel_error = (other_count - R) / R) %>%
  ggplot(aes(x=rel_error, fill=algo)) +
...
correctness %>%
  ...
  summarize(count = n()) %>%
  ungroup %>%
  spread(algo, count, fill = 0) %>%
  gather(algo, other_count, ccrank, crank, rank, rej, R2) %>%
  mutate(rel_error = kimisc::coalesce.na((other_count - R) / (other_count + R) * 2, 0)) %>%
  group_by(prob, algo, r, n) %>%
  summarize(rel_error_sq = sum(rel_error * rel_error)) %>%
  ungroup %>%
  ggplot(aes(x=algo, y=rel_error_sq)) +
